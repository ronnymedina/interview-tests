...
def test_build_llm_uses_configured_model_and_key(monkeypatch):
    """build_llm arma el modelo del proveedor+modelo de config.CHAT_MODEL (sin tocar red)."""
    monkeypatch.setattr(config, "GEMINI_API_KEY", "fake-key-for-construction")
    monkeypatch.setattr(config, "CHAT_MODEL", "google_genai:gemini-2.5-flash")
    llm = build_llm()
